...
/*
 * perlin-ascii — High-performance terminal ASCII noise screensaver
 *
 * Features:
 *   - Smooth noise grid with bilinear interpolation
 *   - 5-level Unicode block character ramp: ' ' ░ ▒ ▓ █
 *   - Half-width resolution with double characters for square pixels
...
// Block character ramp (UTF-8 encoded)
...
static void render_frame(void) {
    ...
    for (int y = 0; y < term_height; y++) {
        ...
        for (int x = 0; x < calc_width; x++) {
            ...
            int p_idx = (val * CHARS_LEN) / 256;
            if (p_idx >= CHARS_LEN) p_idx = CHARS_LEN - 1;
            if (p_idx < 0) p_idx = 0;

            const char *ch = CHARS[p_idx];
            fb_append_str(ch);
            fb_append_str(ch);
        }
